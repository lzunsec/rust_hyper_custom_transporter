    ...
    Bs::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    fn poll_catch(
        ...
        let i = self.poll_inner(cx, should_shutdown);
        println!("after poll_inner");
        match i {
            Poll::Pending => {
                println!("poll_inner PENDING");
            },
            _ => {
                println!("poll_inner NOT PENDING");
            }
        }
        let r= Poll::Ready(ready!(i).or_else(|e| {
            // An error means we're shutting down either way.
            // We just try to give the error to the user,
            // and close the connection with an Ok. If we
            // cannot give it to the user, then return the Err.
            println!("poll_catch e: {}", e);
            self.dispatch.recv_msg(Err(e))?;
            Ok(Dispatched::Shutdown)
        }));
        println!("poll_catch end");
        r
    }
    ...
    fn poll_inner(
        ...
        println!("after poll_loop");

        if self.is_done() {
            println!("self.is_done()");
    ...
    fn poll_loop(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::Result<()>> {
        ...
        for _ in 0..16 {
            ...
            if !self.conn.wants_read_again() {
                println!("!self.conn.wants_read_again()");

                //break;
                return Poll::Ready(Ok(()));
            }
            println!("poll_loop end2");
        }
    ...
    fn poll_read(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::Result<()>> {
        println!("POLL_READ!!!!!");
        loop {
            println!("POLL_READ LOOP!!!!!");
            if self.is_closing {
                println!("is closing");
                return Poll::Ready(Ok(()));
            } else if self.conn.can_read_head() {
                println!("can_read_head YES!");
                ready!(self.poll_read_head(cx))?;
            } else if let Some(mut body) = self.body_tx.take() {
                println!("self.body_tx.take() ");
    ...
    fn poll_read_head(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::Result<()>> {
        ...
        match ready!(self.dispatch.poll_ready(cx)) {
            ...
            Err(()) => {
                ...
                println!("self.dispatch.poll_ready(cx) err");
                self.close();
                return Poll::Ready(Ok(()));
            }
        }
    ...
    fn poll_write(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::Result<()>> {
        println!("poll_write");
        loop {
            println!("poll_write LOOP");
            if self.is_closing {
                println!("self.is_closing");
                return Poll::Ready(Ok(()));
            } else if self.body_rx.is_none()
                && self.conn.can_write_head()
                && self.dispatch.should_poll()
            {
                println!("if self.body_rx.is_none()
                && self.conn.can_write_head()
                && self.dispatch.should_poll()");

                if let Some(msg) = ready!(Pin::new(&mut self.dispatch).poll_msg(cx)) {
                    println!("let Some(msg)");
                    ...
                    if let Some(full) = crate::body::take_full_data(&mut body) {
                        println!("take_full_data");
                        self.conn.write_full_msg(head, full);
                        return Poll::Ready(Ok(()));
                    }

                    let body_type = if body.is_end_stream() {
                        println!("body.is_end_stream");

                        self.body_rx.set(None);
                        None
                    } else {
                        println!("body.is_end_stream ELSE");
                        let btype = body
                            .size_hint()
                            .exact()
                ...
                println!("!self.conn.can_buffer_body()");
