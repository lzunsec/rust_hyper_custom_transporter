    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    fn send_request(
        ...
        println!("before connection_for");
        let conn = self.connection_for(pool_key);
        println!("AFTER connection_for");
    ...
    fn connection_for(
        ...
        let a = future::select(checkout, connect).then(move |either| match either {
            ...
            Either::Left((Ok(checked_out), connecting)) => {
                println!("begin connection_for future outcome 1");
                ...
                let a = Either::Left(future::ok(checked_out));
                println!("end connection_for future outcome 1");
                a
            }
            // Connect won, checkout can just be dropped.
            Either::Right((Ok(connected), _checkout)) => {
                println!("begin connection_for future outcome 2");

                let a = Either::Left(future::ok(connected));
                println!("end connection_for future outcome 2");
            ...
            Either::Left((Err(err), connecting)) => Either::Right(Either::Left({
                println!("begin connection_for future outcome 3");
                let a = if err.is_canceled() {
                    Either::Left(connecting.map_err(ClientError::Normal))
                } else {
                    Either::Right(future::err(ClientError::Normal(err)))
                };
                println!("end connection_for future outcome 3");
                a
            })),
            Either::Right((Err(err), checkout)) => Either::Right(Either::Right({
                println!("begin connection_for future outcome 4");

                let a = if err.is_canceled() {
                    println!("is canceled");
                    Either::Left(checkout.map_err(ClientError::Normal))
                } else {let body = Body::empty();
                    println!("is NOT canceled");
                    println!("is closed? {}", err.is_closed());
                    Either::Right(future::err(ClientError::Normal(err)))
                };
                println!("end connection_for future outcome 4");
                a

            })),
        });
        println!("END connection_for future");
        a
    }
        ...
        hyper_lazy(move || {
            ...
            Either::Left(
                connector
                    ...
                    .and_then(move |io| {
                        println!("hyper lz connector and then!");
